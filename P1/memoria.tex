...
\begin{algorithmic}[1]
\Procedure{getEnemies}{data,dClasses,example,exampleClass}
\BState \emph{loop} : $i = 0,...,data.size$
\If {dClasses(i) = exampleClass}
$enemies \gets data(i)$
\EndIf
\State \textbf{return} enemies
\EndProcedure
\end{algorithmic}
...
\begin{algorithmic}[1]
\Procedure{greedyRelief}{data, classes}
\State $w \gets 0$
\BState \emph{loop}: $i = 0,...,data.size$
\State $friends \gets getFriends$
\State $enemies \gets getEnemies$
\State $closestFriend \gets min(distance(friends,data(i))$
\State $closestEnemy \gets min(distance(enemies,data(i))$
\State $w \gets w + |data(i) - closestEnemy| - |data(i)-closestFriend|$
\BState \emph{endloop}
\State $w \gets normalize(w)$
